...
sub get_cb ($this, $url, %params) {
  return sub {
    my ($cb) = @_;
    $this->{ua}->get($url, \%params, sub ($, $tx) { $cb->($this->_new_response($tx->res)) });
    return;
  };
}
...
sub post {
  ...
  return $this->_new_response($this->{ua}->post($url, $params, (defined ${$content} ? ${$content} : ()))->res);
}
...
sub post_cb {
  ...
  return sub {
    my ($cb) = @_;
    $this->{ua}->post(
      $url, $params, (defined ${$content} ? ${$content} : ()), sub ($, $tx) { $cb->($this->_new_response($tx->res)) });
    return;
  };
}
...
sub post_p {
  my ($this, $url, $content, $params) = &UserAgent::Any::Impl::_get_post_args;
  return $this->{ua}->post_p($url, $params, (defined ${$content} ? ${$content} : ()))
